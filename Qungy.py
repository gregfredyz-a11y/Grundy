...
KEYS_PER_PAGE = 128
LOOKUP_DELAY = 0.05        # Pacing delay between API threads
DISPLAY_DELAY = 0.01       # Tiny pause to let human eyes read the terminal text stream
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if raw_text.isdigit():
                ...
                # --- INTEGRATED DIAGNOSTIC LOG MODULE ---
                with print_lock:
                    sys.stdout.write(f"  \033[93m[via Network API]\033[0m Checked: {address} | Bal: {balance_satoshis} sat ({addr_type})\n")
                    sys.stdout.flush()
...
def generate_local_page(page_num, block_name):
    """Generates keys, formats them for the screen, and prints them in a clean terminal layout."""
    ...
    for i in range(KEYS_PER_PAGE):
        ...
        try:
            ...
            # Submitting tasks to background workers
            ...
            with print_lock:
                ...
                sys.stdout.write(f" ├── Uncompressed: {addr_uncomp}\n")
                sys.stdout.write(f" └── Compressed:   {addr_comp}\n\n")
                sys.stdout.flush()

            time.sleep(LOOKUP_DELAY + DISPLAY_DELAY)
        except Exception:
            continue
...
def run_range_scanner(start, end, block_name):
    ...
    while current_chunk_start <= end:
        ...
        # Static progress banner displayed periodically between blocks
        with print_lock:
            print(f"\n\033[92m[+]\033[0m Completed block: {block_name} | Page Chk: {current_chunk_start - 1}/{end} | Keys: {total_keys_checked} | API Threads: {active_balance_threads}\n")
...
print(f"[+] Streaming generated keys directly to terminal layout...\n")
time.sleep(1.5)
